  ...
  return (
    <>
      ...
      <Link href={"#/en-US/search"}
        className={iconFontClassName + " py-4 px-5 mr-3 text-3xl md:text-xl hover:bg-yellow dark:hover:text-black uppercase block lg:hidden"}>{"\ue913"}</Link>
    </>
  )
}
  ...
  return (
    <nav className="sticky top-0 z-50 shadow-md">
      ...
      <div className="bg-white dark:bg-gray-800 flex items-center h-[80px]">
        <button
          onClick={toggle} className={iconFontClassName + " text-4xl px-3flex items-center text-center md:hidden w-[60px] h-[60px] mr-4"}>
          {isOpen ? "X" : "\ue916"}
        </button>
        {!isOpen && <Link
          href="/en-US"
