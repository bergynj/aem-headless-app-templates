...
export interface AdventureCollection {
    name: string;
    slug: string;
    predicate: (adventure: Adventure) => boolean;
}

export const adventureCollections: AdventureCollection[] = [
    {
        name: 'All',
        slug: 'all',
    ...
    },
    {
        name: 'Sport',
        slug: 'sport',
        predicate: (adventure) => (adventure.title?.includes('Ski') || adventure.title?.includes('Cycling') || adventure.title?.includes('Surf')) ?? false,
    },
    {
        name: 'Summer',
        slug: 'summer',
    ...
    },
    {
        name: 'Winter',
        slug: 'winter',
        predicate: (adventure) => adventure.title?.includes('Ski') ?? false,
    },
    {
        name: 'Most Popular',
        slug: 'popular',
        predicate: (adventure) => (adventure.title?.includes('surf')
            ...
            || adventure.title?.includes('Cycling')) ?? false,
    },
];

export const getAdventures = cache(async (lang?: string) => {
    // await new Promise(r => setTimeout(r, 10000));
    const client = AdventureClient.fromEnv();
    const res = await client.getAllAdventures();
    return res?.data?.adventureList?.items as Adventure[];
});

export class AdventureClient {
    static __envClient: AdventureClient;
    aemHeadlessClient: any;
    ...
    constructor({ serviceURL, endpoint }: { serviceURL: string; endpoint: string }) {
    ...
    async getAdventurePaths() {
        ...
        const adventures = (res?.data?.adventureList?.items || []) as Adventure[];
